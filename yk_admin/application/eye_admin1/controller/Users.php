...
class Users extends Base{
 ...
 //    }
	...
	//搜索功能
	function users_search($date1,$date2,$search,$u_type,$each=15,$page=1){
		//联合查询条件: date1 && date2 =>日期  search=>昵称搜索
		$cond=[];
		$strtime=strtotime($date1);
		$endtime=strtotime($date2);
		$endtime=$endtime+59;
		$cond['i.time']=['between',[$strtime,$endtime]];
		$cond['i.type']=$u_type;
		if ($search!=""){
			$cond['i.nickname']=['like','%'.$search.'%'];
		}
		// mp($cond);
		$item = model("user")->users_search($cond);
		$amount = count($item);
		$maxPage =ceil($amount/$each);
		$page =getPage($maxPage,$page);
		$pages =getPages($maxPage,$page);
		$href = 'users_search?';
		$items =model("user")->users_searchPage($cond,$each,$page);
		for ($i=0;$i<count($items);$i++){ 
			$items[$i]['time']=date("Y-m-d H:i:s",$items[$i]['time']);
		}
		if (!$items) {
	...
	//冻结用户的功能
	function users_frozen($id,$frozen,$u_type){
		if ($frozen==0) {
			$cond =model("user");
			$cond->save([
			'frozen'=>1	
		...
		if ($u_type==1) {
			return $this->redirect('users_B1');
		}elseif($u_type==2){
			return $this->redirect('users_B1_2');
		}elseif($u_type==3){
			return $this->redirect('users_B2_1');
		}elseif($u_type==4){
			return $this->redirect('users_B2_2');
		}else{
			return $this->redirect('users_C1');
		}
	}
